...
#%%
# BSF T1W-GD DIAGNOSE Model
current_model = "BSF_t1gd_diag"
...
#%%
# BSF T2W DIAGNOSE Model
current_model = "BSF_t2_diag"
label_column = "class_label"
...
#%%
# BSF fused T1W-GD and T2W DIAGNOSE Model
current_model = "BSF_t1gd_and_t2_diag"
label_column = "class_label"
...
#%%
# ResNet (2+1)D on T1W-GD, DIAGNOSE Model
current_model = "123"
label_column = "loc_label"


#%%
# ResNet (2+1)D on T2W, DIAGNOSE Model


#%%
# ResNet (2+1)D on fused T1W-GD and T2W, DIAGNOSE Model
...
for id, batch_data in enumerate(loader[2]):
    ...
    #print(f"DATA SHAPE: {data.shape}")
    pred = model(data)
    #all_preds.append(pred.tolist()[0]) # Assuming batch size is 1

    outputs_np = torch.softmax(pred, dim=1).detach().cpu().numpy()
...
df_logits = pd.DataFrame(all_preds).to_csv(Path(PREDICTION_PATH, current_model+".csv"), index=False)
# %%
